...
We can assert that English-speaking tourists value staff friendliness and location convenience concerning transport, but are concerned about any kind of decline in room quality both visually and regarding the smell and air quality, being quick to judge any sort of remains of cigarette smell.
...
The extrapolated Rank-biased Overlap (shown in Table \ref{tab:rbo}) ranged from 0.221 to 0.265 in satisfaction lists at different cutoff points, and from 0.285 to 0.321 in the dissatisfaction lists, which are low similarity values. This means that, while there is some similarity, the preferences are fundamentally different if we consider them as top-weighted, that is, the first elements are the most important in the lists, and therefore in their similarity measurement as well. This confirms our hypothesis \ref{hyp:1}, that the satisfaction and dissatisfaction factors are different for Chinese and English-speaking tourists. 
