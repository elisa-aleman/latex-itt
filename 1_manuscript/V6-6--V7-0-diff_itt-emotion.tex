...
%DIF ADD itt-emotion.tex        Thu Apr  1 04:29:57 2021
%
...
\begin{abstract}
  ...
  \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{With a growing tourism industry in Japan, the importance of analyzing customer expectations and satisfaction factors at a large scale is evident. Furthermore, a cross-culture study is necessary to understand the differences in satisfaction factors of the biggest tourist populations in Japan, Chinese and Western tourists. 
  Therefore, our study }\DIFaddend analyzed expectations and satisfaction factors in Chinese and Western cultures, comparing soft attributes, such as service, with hard attributes, such as location and facilities, and studied different price ranges.
  ...
  We found that Chinese tourists are concerned more with room quality than hospitality, whereas, Western tourists are delighted more by the staff behavior. We also found that the lack of a Chinese-friendly environment for Chinese customers and the smell of cigarettes for Western ones can be disappointing factors of their stay. 
...
\section{Research objective}\label{research_objective}
  ...
  \DIFaddbegin \DIFadd{With the knowledge that cultural background influences expectations in customers, which in turn is the basis for satisfaction, it becomes important to know the difference in difference in factors influencing satisfaction and dissatisfaction between the most differing and numerous tourist populations in a given area. 
}
...
\section{Theoretical background and hypothesis development}\label{theory_hypothesis}
  ...
  \subsection{\DIFdelbegin \DIFdel{Japanese hospitality }\DIFdelend \DIFaddbegin \DIFadd{Cultural influence in expectation }\DIFaddend and \DIFdelbegin \DIFdel{service: }\textit{\DIFdel{Omotenashi}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{satisfaction}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{theory_omotenashi}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{theory_expectations}
\DIFaddend 

    \DIFdelbegin \DIFdel{The spirit of Japanese hospitality, or }\textit{\DIFdel{Omotenashi}}%DIFAUXCMD
\DIFdel{, has roots in the country’s history, and to this day, it is regarded as the highest standard }%DIFDELCMD < \cite[][]{ikeda2013omotenashi, al2015characteristics}%%%
\DIFdel{. There is a famous phrase in customer service in Japan: }\textit{\DIFdel{okyaku-sama wa kami-sama desu}}%DIFAUXCMD
\DIFdel{, meaning ``The customer is god.'' Some scholars say that }\textit{\DIFdel{omotenashi}} %DIFAUXCMD
\DIFdel{originated from the old Japanese art of the tea ceremony in the 16th century, while others found that it originates in the form of formal banquets in the 7th-century }%DIFDELCMD < \cite[][]{aishima2015origin}%%%
\DIFdel{. The practice of high standards in hospitality has survived throughout the years. Presently, it permeates all business practices in Japan, from the cheapest convenience stores to the most expensive ones. Manners, service, and respect towards the customer are taught to workers in their training. High standards are always followed to not fall behind in the competition. In Japanese businesses, including hotels, staff members are trained to speak in }\textit{\DIFdel{sonkeigo}}%DIFAUXCMD
\DIFdel{, or ``respectful language,'' one of the most formal of the Japanese formality syntaxes. They are also trained to bow differently depending on the situation, where a light bow could be used to say ``Please, allow me to guide you.'' Deep bows are used to apologize for any inconvenience the customer could have faced, followed by a very respectful apology. Although the word }\textit{\DIFdel{omotenashi}} %DIFAUXCMD
\DIFdel{can be translated directly as ``hospitality,'' it includes both the concepts of hospitality and service }%DIFDELCMD < \cite[][]{Kuboyama2020}%%%
\DIFdel{. This hospitality culture permeates every type of business with customer interaction in Japan. A simple convenience shop could express all of these hospitality and service standards, which are not exclusive to hotels.  
}%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{It stands to reason that this cultural aspect of hospitality would positively influence customer satisfaction. However, in many cases, other factors such as proximity to a convenience store, transport availability, or room quality might be more critical to a customer.  In this study, we cannot directly determine whether a hotel is practicing the cultural standards of }\textit{\DIFdel{omotenashi}}%DIFAUXCMD
\DIFdel{. Instead, we consider it as a cultural factor that influences all businesses in Japan. We then observe the customers' evaluations regarding service and hospitality factors and compare them to other places and business practices in the world. In summary, we consider the influence of the cultural aspect of }\textit{\DIFdel{omotenashi}} %DIFAUXCMD
\DIFdel{while analyzing the evaluations on service and hospitality factors that are universal to all hotels in any country.
}%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{Therefore, we pose the following research question:
}%DIFDELCMD < 

%DIFDELCMD <     \begin{subrsq}
%DIFDELCMD <     \begin{rsq}
%DIFDELCMD <     \label{rsq:hospitality}
%DIFDELCMD <     %%%
\DIFdel{To what degree are Chinese and Western tourists satisfied with Japanese hospitality factors such as staff behavior or service?
    }%DIFDELCMD < \end{rsq}
%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{However, Japanese hospitality is based on the Japanese culture. Different cultures interacting with it could provide a different evaluation of it. Some might be impressed by it, whereas some might consider other factors more important to their stay in a hotel. This point leads us to a derivative of the aforementioned research question:
}%DIFDELCMD < 

%DIFDELCMD <     \begin{rsq}
%DIFDELCMD <     \label{rsq:hospitality_both}
%DIFDELCMD <     %%%
\DIFdel{Do Western and Chinese tourists have a different evaluation of Japanese hospitality factors such as staff behavior or service?
    }%DIFDELCMD < \end{rsq}
%DIFDELCMD <     \end{subrsq}
%DIFDELCMD < 

%DIFDELCMD <   %%%
\subsection{\DIFdel{Customer satisfaction and dissatisfaction towards individual factors during hotel stay}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{theory_satisfaction}
%DIFDELCMD < 
...
\DIFdel{. Western and Chinese customers then have very different backgrounds and cultures. These varying backgrounds }\DIFdelend \DIFaddbegin \DIFadd{as well }\cite[e.g.][]{MATTILA201910, LEVY2010319, donthu1998cultural}\DIFadd{, such as comparing individualism vs. collectivism, high context vs. low context, uncertainty avoidance, among other factors. While culture as a concept is difficult to quantify, some researchers have tried to use these and more dimensions to measure cultural differences, such as the six dimensions described by }\cite{hofstede1984culture}\DIFadd{, or the nine dimensions of the GLOBE model }\cite[][]{house1999cultural}\DIFadd{.
    ...
    \DIFdelbegin \DIFdel{However, in }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Customer satisfaction and dissatisfaction towards individual factors during hotel stay}}\label{theory_satisfaction}
...
\DIFdel{. However, hard factors, which are }\DIFdelend \DIFaddbegin \subsection{\DIFadd{Japanese hospitality and service: }\textit{\DIFadd{Omotenashi}}}\label{theory_omotenashi}
    ...
    \DIFadd{The spirit of Japanese hospitality, or }\textit{\DIFadd{Omotenashi}}\DIFadd{, has roots in the country’s history, and to this day, it is regarded as the highest standard }\cite[][]{ikeda2013omotenashi, al2015characteristics}\DIFadd{. There is a famous phrase in customer service in Japan: }\textit{\DIFadd{okyaku-sama wa kami-sama desu}}\DIFadd{, meaning ``The customer is god.'' Some scholars say that }\textit{\DIFadd{omotenashi}} \DIFadd{originated from the old Japanese art of the tea ceremony in the 16th century, while others found that it originates in the form of formal banquets in the 7th-century }\cite[][]{aishima2015origin}\DIFadd{. The practice of high standards in hospitality has survived throughout the years. Presently, it permeates all business practices in Japan, from the cheapest convenience stores to the most expensive ones. Manners, service, and respect towards the customer are taught to workers in their training. High standards are always followed to not fall behind in the competition. In Japanese businesses, including hotels, staff members are trained to speak in }\textit{\DIFadd{sonkeigo}}\DIFadd{, or ``respectful language,'' one of the most formal of the Japanese formality syntaxes. They are also trained to bow differently depending on the situation, where a light bow could be used to say ``Please, allow me to guide you.'' Deep bows are used to apologize for any inconvenience the customer could have faced, followed by a very respectful apology. Although the word }\textit{\DIFadd{omotenashi}} \DIFadd{can be translated directly as ``hospitality,'' it includes both the concepts of hospitality and service }\cite[][]{Kuboyama2020}\DIFadd{. This hospitality culture permeates every type of business with customer interaction in Japan. A simple convenience shop could express all of these hospitality and service standards, which are not exclusive to hotels.  
}

    \DIFadd{It stands to reason that this cultural aspect of hospitality would positively influence customer satisfaction. However, in many cases, other factors such as proximity to a convenience store, transport availability, or room quality might be more critical to a customer.  In this study, we cannot directly determine whether a hotel is practicing the cultural standards of }\textit{\DIFadd{omotenashi}}\DIFadd{. Instead, we consider it as a cultural factor that influences all businesses in Japan. We then observe the customers' evaluations regarding service and hospitality factors and compare them to other places and business practices in the world. In summary, we consider the influence of the cultural aspect of }\textit{\DIFadd{omotenashi}} \DIFadd{while analyzing the evaluations on service and hospitality factors that are universal to all hotels in any country.
}

    \DIFadd{Therefore, we pose the following research question:
}

    \begin{subrsq}
    \begin{rsq}
    \label{rsq:hospitality}
    \DIFadd{To what degree are Chinese and Western tourists satisfied with Japanese hospitality factors such as staff behavior or service?
    }\end{rsq}

    \DIFadd{However, Japanese hospitality is based on the Japanese culture. Different cultures interacting with it could provide a different evaluation of it. Some might be impressed by it, whereas some might consider other factors more important to their stay in a hotel. This point leads us to a derivative of the aforementioned research question:
}

    \begin{rsq}
    \label{rsq:hospitality_both}
    \DIFadd{Do Western and Chinese tourists have a different evaluation of Japanese hospitality factors such as staff behavior or service?
    }\end{rsq}
    \end{subrsq}

    \DIFadd{Staff behavior, hospitality and service are all soft attributes of a hotel. That is, they are non-physical attributes of the hotel, and as such they are practical to change through changes in management. While it is important to know this, it is not known if the cultural differences between Chinese and Western tourists also influences other expectations and satisfaction factors, such as hard factors of a hotel. Hard factors are attributes }\DIFaddend uncontrollable by the hotel staff, \DIFaddbegin \DIFadd{which }\DIFaddend can play a part in the customers' choice behavior and satisfaction. Examples of these factors include the hotel's surroundings, location, language immersion of the country as a whole, or touristic destinations, and the hotel's integration with tours available nearby, among other factors. \DIFaddbegin \DIFadd{Previous research focuses more on these soft attributes, with little focus on hard attributes, if only focusing on facilities }\cite[e.g.][]{shanka2004, choi2001}\DIFadd{. Because of this gap in knowledge, we decided to analyze the differences in cultures regarding both soft and hard attributes of a hotel.
}\DIFaddend 

    This leads to \DIFdelbegin \DIFdel{another couple of }\DIFdelend \DIFaddbegin \DIFadd{two of our }\DIFaddend research questions:
